...
class BubbleExtractorImageTransformation(ImageTransformationInterface):
    ...
    @staticmethod
    def __sort_points(pts, left_right=True):
    ...
    @staticmethod
    def __extract_answer_bubble(image, bubbles):
        bubbled = -np.inf, 0
    ...
    def __set_answers(self, image, n_choices=5):
        for question_number, contour_row in enumerate(np.arange(0, len(self._question_cnts), n_choices), start=1):
            left_right_bubbles = self.__sort_points(self._question_cnts[contour_row:contour_row+n_choices])
    ...
    def __call__(self, image: npt.ArrayLike) -> list[npt.ArrayLike]:
        ...
        self._question_cnts = self.__sort_points(self._question_cnts, left_right=False)
